...
SoundHandle AudioSystem::PlaySound(const std::string& soundName, bool looping, int volume)
{
    ...
    auto last = mLastHandle;
    ++mLastHandle;
    int free = -1;
    SoundHandle * equal = nullptr;
    auto lastPaused = new SoundHandle();
    ...
    for (int i = 0 ; i < mChannels.size(); i++)
    {
        ...
        if (last > mChannels[i] && mHandleMap[mChannels[i]].mSoundName != "Music.ogg")
        {
            last = mChannels[i];
        }

        if (mHandleMap[mChannels[i]].mIsPaused)
        {
            if (!lastPaused->IsValid() || mChannels.at(i) < *lastPaused)
            {
                lastPaused = &mChannels.at(i);
            }
        }

        if (mHandleMap[mChannels[i]].mSoundName == soundName)
        {
            equal = &mChannels.at(i);
        }
    }
    ...
    else
    {
        if (equal != nullptr)
        {
            info = mHandleMap.at(*equal);
            mHandleMap.erase(*equal);
            mHandleMap.emplace(mLastHandle, info);
        }
        else if (!lastPaused->IsValid()) {
            info = mHandleMap.at(*lastPaused);
            mHandleMap.erase(*lastPaused);
            mHandleMap.emplace(mLastHandle, info);
        }
        else
        {
            info = mHandleMap.at(last);
            mHandleMap.erase(last);
            mHandleMap.emplace(last, info);
        }
    }
    ...
    Mix_PlayChannel(info.mChannel, sound, looping);
...
void AudioSystem::SetVolume(SoundHandle sound, int volume)
{
    if(mHandleMap.find(sound) == mHandleMap.end())
    {
        return;
...
void AudioSystem::SetVolume(std::string name, int volume)
{
    int channel = -1;
    for (auto s : mChannels)
    {
        if (s.IsValid())
        {
            if (mHandleMap[s].mSoundName == name)
            {
                channel = mHandleMap[s].mChannel;
                break;
            }
        }
    }
